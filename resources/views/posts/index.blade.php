            ...
            <div onclick="location.href='/posts/{{ $post->id }}';" style="cursor: pointer;" class="xl:w-1/4 md:w-1/2 p-4">
                <div class=" bg-gray-800 p-6 rounded-lg">
                    @if (isset($post->image_name))
                        ...
                        src={{ Storage::disk('public')->url($post->image_name) }}alt="content">
                    @else
                        <img class="h-40 rounded w-full object-cover object-center mb-6"
                        src={{ Storage::disk('public')->url('noImage.jpg')}}  alt="content">
                    @endif

                    <h3 class="tracking-widest text-purple-500 text-xs font-medium title-font">by
                        {{ App\Models\User::find($post->user_id)->username }}</h3>
